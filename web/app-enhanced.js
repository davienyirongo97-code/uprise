...
function parseMalawiIDBarcode(barcodeData) {
    ...
    // The format may vary, but commonly includes: ID Number, Name, DOB, etc.
    // This is a sample parser - adjust based on actual Malawi ID barcode format
    
    try {
        // Example format: ID|SURNAME|FIRSTNAME|DOB|GENDER|DISTRICT
        // Or it might be a single string with fixed positions
        
        let parsedData = {};
        
        // Try parsing as delimited data (common format)
        if (barcodeData.includes('|') || barcodeData.includes(',')) {
            const delimiter = barcodeData.includes('|') ? '|' : ',';
            const parts = barcodeData.split(delimiter);
            
            // Adjust indices based on actual Malawi ID format
            parsedData = {
                nationalId: parts[0] || '',
                surname: parts[1] || '',
                firstName: parts[2] || '',
            ...
            // Adjust positions based on actual format
            parsedData = {
        ...
        showNotification('success', 'ID information retrieved successfully! Please verify and complete remaining fields.');
        
        // Move to step 2 (Personal Information)
        currentStep = 2;
        showStep(2);
